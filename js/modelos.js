(() => {
  const state = { models: [], category: 'Todos', purpose: 'Todos', search: '', sort: 'featured' };
  ...
  const purposes = () => ['Todos', ...new Set(state.models.map(model => model.purpose).filter(Boolean))];
  ...
  function renderFilters() {
    filters.innerHTML = `<div class="filter-group"><span class="filter-label">Categoria</span>${categories().map(category => `<button class="filter ${category === state.category ? 'active' : ''}" data-category="${escapeHtml(category)}">${escapeHtml(category)}</button>`).join('')}</div><div class="filter-group"><span class="filter-label">Finalidade</span>${purposes().map(purpose => `<button class="filter ${purpose === state.purpose ? 'active' : ''}" data-purpose="${escapeHtml(purpose)}">${escapeHtml(purpose)}</button>`).join('')}</div>`;
  }
  ...
  function visibleModels() {
    ...
    return state.models.filter(model => {
      ...
      const purposeOk = state.purpose === 'Todos' || model.purpose === state.purpose;
      const text = [model.name, model.category, model.purpose, model.description, ...(model.tags || [])].join(' ').toLowerCase();
      return categoryOk && purposeOk && text.includes(query);
  ...
  function render() {
    ...
    grid.innerHTML = models.length ? models.map(model => `<article class="model-card ${model.featured ? 'is-featured' : ''}"><div>${visualFor(model)}<div class="model-top"><span class="model-category">${escapeHtml(model.category)}</span><span>${model.featured ? '★' : '—'}</span></div><h2>${escapeHtml(model.name)}</h2><p>${escapeHtml(model.description)}</p><div class="model-tags">${model.purpose ? `<span class="model-tag">${escapeHtml(model.purpose)}</span>` : ''}${(model.tags || []).map(tag => `<span class="model-tag">${escapeHtml(tag)}</span>`).join('')}</div></div><div class="model-actions"><button data-view="${escapeHtml(model.id)}">Visualizar</button><button class="secondary" data-use="${escapeHtml(model.id)}">Usar modelo</button></div></article>`).join('') : '<div class="empty-models">Nenhum modelo encontrado.</div>';
  }

  function openModal(model) {
    modalRoot.innerHTML = `<div class="modal-backdrop" role="dialog" aria-modal="true"><div class="modal"><div class="modal-head"><div>${visualFor(model)}<p class="kicker dark">${escapeHtml(model.category)}${model.purpose ? ` · ${escapeHtml(model.purpose)}` : ''}</p><h2>${escapeHtml(model.name)}</h2></div><button class="modal-close" aria-label="Fechar">×</button></div><p>${escapeHtml(model.description)}</p><pre>${escapeHtml(model.content)}</pre><div class="model-actions"><button data-copy="${escapeHtml(model.id)}">Copiar modelo</button><button class="secondary" data-use="${escapeHtml(model.id)}">Usar modelo</button></div></div></div>`;
  }

  function useModel(model) {
    try { localStorage.setItem('nexa-selected-model', JSON.stringify({ source:'nexa-model-library', modelId:model.id, name:model.name, category:model.category, purpose:model.purpose || '', content:model.content, selectedAt:new Date().toISOString() })); } catch (_) {}
  ...
  document.addEventListener('click', event => {
    ...
    const purpose = event.target.closest('[data-purpose]'); if (purpose) { state.purpose = purpose.dataset.purpose; renderFilters(); render(); return; }
    ...
    const use = event.target.closest('[data-use]'); if (use) { const model = findModel(use.dataset.use); if (model) useModel(model); return; }
    const copy = event.target.closest('[data-copy]'); if (copy) { const model = findModel(copy.dataset.copy); if (model) copyModel(model); return; }
